...
func Read(Data []byte, Connection *wsc.Wsc) {
	...
	case "downloadFile":
		{
			...
			if err != nil {
				...
				msgpack.ForcePathObject("HWID").SetAsString(unmsgpack.ForcePathObject("HWID").GetAsString())
	...
	case "shell":
		{
			Controler_HWID := unmsgpack.ForcePathObject("HWID").GetAsString()
			//fmt.Println(Controler_HWID)
	...
	case "shellWriteInput":
		{
			...
			m <- []byte(strings.Replace(unmsgpack.ForcePathObject("WriteInput").GetAsString(), "", "", -1) + "\n")
			return
		}
